function planetWeight(num){
    ...
    if(!isNaN(n1)){
        if(num==1){
            ...
            document.getElementById("p1").innerHTML="Your weight on SUN is ";
            document.getElementById("p2").innerHTML=Math.round(n1*27.01)+" kgs ";
        }
        else{
            document.getElementById("p1").innerHTML="Your weight on Moon is ";
            document.getElementById("p2").innerHTML=Math.round(n1*0.166)+" kgs ";
        }
    }
    else{
        document.querySelectorAll('p1').forEach(item => item.remove());
